...
Renderer* PrefabManager::GetRenderer(string name)
{
	for each (Renderer * renderer in m_Renderers)
		if (renderer->GetName() == name)
			return renderer;

	printf("ERROR: can not find renderer with name: %s\n", name.c_str());
	exit(1);
	return NULL;
}
